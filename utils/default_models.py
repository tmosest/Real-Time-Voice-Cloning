...
default_models = {
    ...
    "synthesizer": ("https://drive.google.com/uc?export=download&id=1EqFMIbvxffxtjiVrtykroF6_mUh-5Z3s", None),
...
def ensure_default_models(models_dir: Path):
    ...
    for model_name, (url, size) in default_models.items():
        ...
        if target_path.exists():
            if size is not None and target_path.stat().st_size != size:
    ...
    for thread, target_path, size in jobs:
        ...
        assert target_path.exists() and (size is None or target_path.stat().st_size == size), \
